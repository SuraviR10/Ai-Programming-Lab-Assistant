...
from database import get_db, Submission, User, LabActivity
...
def run_code(req: RunRequest):
    ...
    If GCC fails -> Sends compiler error to Groq LLM (unless exam mode).
    If GCC succeeds -> Returns output stdout.
    """
    result = compile_and_run(req.code, input_data=req.input_data)

    if not result["success"]:
        # GCC Compilation error -> Call Groq AI Service (mode policy enforced inside service)
